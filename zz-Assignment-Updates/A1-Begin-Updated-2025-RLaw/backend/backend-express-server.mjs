...
// GET endpoint to list saved-jobs and search for saved-jobs
...
// DELETE endpoint to delete a single saved-job by id
app.delete('/saved-jobs/:id', async (req, res) => {
  try {
    ...
    if (!savedJob) {
      console.log(`saved-job with ID ${id} not found, no deletion performed.`);
    ...
    if (savedJob) {
      ...
      if (deletedJob) {
        ...
        console.log(`saved-job with ID ${id} deleted successfully.`);
      } else {
        res.status(500).json({ message: 'saved-job NOT deleted successfully.' });
        console.log(`saved-job with ID ${id} NOT deleted successfully.`);
      }
    }
  } catch (error) {
    console.error('Error fetching job:', error);
